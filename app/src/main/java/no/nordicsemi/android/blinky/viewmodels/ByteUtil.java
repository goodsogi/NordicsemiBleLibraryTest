...
//  private final String TAG = this.getClass().getName();
   ...
   * @return
   */
  public static String mf_str_toHexString(byte b) {
    String hexByte;
   ...
   * @return
   */
  public static String mf_str_toHexString(byte[] bytes) {
   ...
   * @return
   */
  public static String mf_str_toHexString(byte[] bytes, int length) {
   ...
   */
  public static boolean mf_b_compare(byte[] ba1, byte[] ba2) {
  ...
  /**
   * byte[]를 short 변경합니다.(2개 바이트를 넘을 경우 제대로 동작하지 않습니다.)
   *
   * @param ba
   * @return
   */
  public static short mf_s_toShort(byte[] ba) {
    short result = 0;
    for (int i = 0; i < ba.length; i++) {
      result = (short) ((result << 8) | (ba[i] & 0x00FF));
    }
    return result;
  }
   ...
   * @return
   */
  public static int mf_i_toInt(byte[] ba) {
    int result = 0;
    for (int i = 0; i < ba.length; i++) {
   ...
   * @return
   */
  public static byte[] mf_cp_expend(byte[] val, int length) {
   ...
   * @return
   */
  public static int mf_i_unsignedByteToInt(byte b) {
    return (int) b & 0xFF;
  }
   ...
   * @return
   */
  public static byte mf_c_CRC8Byte(byte cCrc, byte cData) {
   ...
   * @return
   */
  public static byte mf_c_getCRC8(byte crc_in, byte[] buff, int count) {
    byte rtn;
    byte inx;
   ...
   */
  public static byte[] mf_cp_Int2ByteArray(int val) {
    return new byte[]{
      (byte) (val >>> 24),
      (byte) (val >>> 16),
      (byte) (val >>> 8),
  ...
  public static byte[] mf_cp_Long2ByteArray(long val) {
    return new byte[]{
      (byte) (val >>> 56),
      (byte) (val >>> 48),
      (byte) (val >>> 40),
   ...
   */
  public static byte[] mf_cp_Int2ByteArray(int val, int len) {
    if (len > 4 || len < 0) {
      return null;
    }
...
//    }
   ...
   * @return XOR checksum
   */
  public static byte mf_c_getCheckSum(byte[] packet) {
    byte bcc = 0x00;
   ...
   * @return
   */
  public static String mf_str_byteArrayToString(byte[] strArray) {
    int idx = 0;
   ...
   * @return
   */
  public static byte[][] mf_cp_splitBytes(byte[] fullpacket, int size) {
   ...
   */
  public static byte[] mf_cp_copyOf(byte[] src, int offset, int length) {
    byte[] result = new byte[length];
  ...
  /**
   * byte array의 CRC16을 계산결과를 byte array(byte[2])로 리턴한다.
   *
   * @param data - 계산할 byte array
   * @return 계산된 CRC16 byte array(byte[2])
   */
  public static byte[] mf_cp_makePacketCRC(byte[] data) {
    int crc = 0xE012;
    ...
    for (int i = 0; i < data.length; i++) {
      //crc ^= data[i];
      cByte = mf_cp_Int2ByteArray(crc);
      cByte[3] ^= (byte) data[i];
      crc = mf_i_toInt(cByte);
    ...
    //Logger.d(TAG, "MakePacketCRC() 02 " + Integer.toString(crc));
    result[0] = (byte) (crc >> 8);
    result[1] = (byte) crc;
